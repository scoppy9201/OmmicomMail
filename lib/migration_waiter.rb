...
class MigrationWaiter
  ...
  ATTEMPTS = OmmicomMail::Config.migration_waiter.attempts
  SLEEP_TIME = OmmicomMail::Config.migration_waiter.sleep_time
  ...
  class << self
    ...
    def wait
      ...
      loop do
        ...
        if pending_migrations.zero?
          OmmicomMail.logger.info "no pending migrations, continuing"
          return
        end
        ...
        if attempts_remaining.zero?
          OmmicomMail.logger.info "#{pending_migrations} migration(s) are still pending after #{ATTEMPTS} attempts, exiting"
          Process.exit(1)
        else
          OmmicomMail.logger.info "waiting for #{pending_migrations} migration(s) to be applied (#{attempts_remaining} remaining)"
          sleep SLEEP_TIME
        end
      end
    end
    ...
    def wait_if_appropriate
      ...
      return unless OmmicomMail::Config.migration_waiter.enabled?
