...
describe Domain do
  ...
  describe "#spf_record" do
    it "returns the SPF record" do
      expect(domain.spf_record).to eq "v=spf1 a mx include:#{OmmicomMail::Config.dns.spf_include} ~all"
    end
  end
  ...
  describe "#dkim_identifier" do
    ...
    context "when the domain has a dkim identifier string" do
      ...
      it "returns the DKIM identifier" do
        expect(domain.dkim_identifier).to eq "#{OmmicomMail::Config.dns.dkim_identifier}-#{domain.dkim_identifier_string}"
      end
    end
  end
  ...
  describe "#dkim_record_name" do
    ...
    context "when the domain has a dkim identifier string" do
      ...
      it "returns the DKIM identifier" do
        expect(domain.dkim_record_name).to eq "#{OmmicomMail::Config.dns.dkim_identifier}-#{domain.dkim_identifier_string}._domainkey"
      end
    end
  end
  ...
  describe "#return_path_domain" do
    it "returns the return path domain" do
      expect(domain.return_path_domain).to eq "#{OmmicomMail::Config.dns.custom_return_path_prefix}.#{domain.name}"
    end
  end
  ...
  describe "#dns_verification_string" do
    ...
    it "returns the DNS verification string" do
      expect(domain.dns_verification_string).to eq "#{OmmicomMail::Config.dns.domain_verify_prefix} #{domain.verification_token}"
    end
  end
  ...
  describe "#resolver" do
    context "when the local nameservers should be used" do
      before do
        allow(OmmicomMail::Config.postal).to receive(:use_local_ns_for_domain_verification?).and_return(true)
      end
