  ...
  describe Client do
    ...
    describe "HELO" do
      it "returns the hostname" do
        ...
        expect(client.handle("HELO: test.example.com")).to eq "250 #{OmmicomMail::Config.postal.smtp_hostname}"
    ...
    describe "EHLO" do
      ...
      context "when TLS is enabled" do
        it "returns capabilities include starttls" do
          allow(OmmicomMail::Config.smtp_server).to receive(:tls_enabled?).and_return(true)
          expect(client.handle("EHLO test.example.com")).to eq ["250-My capabilities are",
                                                                "250-STARTTLS",
                                                                "250 AUTH CRAM-MD5 PLAIN LOGIN",]
        end
