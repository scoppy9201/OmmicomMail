  ...
  describe Client do
    ...
    describe "DATA" do
      ...
      it "adds a received header for itself" do
        ...
        Timecop.freeze do
          client.handle("DATA")
          expect(client.headers["received"]).to include "from test.example.com (1.2.3.4 [1.2.3.4]) by #{OmmicomMail::Config.postal.smtp_hostname} with SMTP; #{Time.now.utc.rfc2822}"
        end
      end
        ...
        it "logs content" do
          Timecop.freeze do
            ...
            expect(client.instance_variable_get("@data")).to eq <<~DATA
              Received: from test.example.com (1.2.3.4 [1.2.3.4]) by #{OmmicomMail::Config.postal.smtp_hostname} with SMTP; #{Time.now.utc.rfc2822}\r
              Subject: Test\r
              \r
